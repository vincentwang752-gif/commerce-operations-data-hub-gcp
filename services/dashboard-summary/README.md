...
默认只读现有 Google Sheets 的订单、红人、归因触点、客户生命周期和 GA4 日数据，写入**另一张专用汇总表**。不读写广告账户，不修改原始数据，不触发邮件。独立看板汇总输出不含客户姓名、邮箱、地址或订单ID。

## 可选：同表业务关联视图

设置 `BUSINESS_VIEWS_ENABLED=true` 后，额外读取客户和内容资产，在**原明细工作簿内部**生成三个页签：客户跟进总览、订单来源总览、红人效果总览。这些明细视图包含客户个人信息，沿用原工作簿受限权限，不写入独立汇总表，不得公开。

- 原始页签不改、不删。新视图放在最前面，冻结两列及表头，带筛选、字段注释和更新时间。人工修改应回到原始表，下次刷新会替换生成视图。
- 客户订单从订单的客户链接重新计算；无链接时仅允许唯一 Shopify 客户ID匹配，不用姓名或模糊邮箱猜测。已有坏链接不自动改绑。
- 订单ID重复时整体排除，重复数量在汇总表数据说明披露。关联缺失的生命周期不会硬分配给客户，数据说明单列异常数量。
- 金额按币种分别汇总，仅覆盖已同步订单。取消/未付款不计入；退款单缺退款金额时不计金额。历史普通已付款订单空白退款按0。全额退款订单仍属于已付款历史，并非留存购买数。
- 问卷完成与延保审核分别展示；多条生命周期记录不把不同订单的阶段强行拼成一次完整调查。不自动发送邮件或批准延保。
- 红人涉及订单与最终归因订单分别展示；多最终触点或多红人冲突不分配成交金额。内容只使用已有明确记录链接，不根据同一红人推断某条视频带来订单。
- GA4日汇总不能还原客户旅程；广告系列日数据不分摊到具体内容或广告。广告与内容的进一步连接仍需要明确素材/内容ID，不造匹配。
- 首次创建写入 developerMetadata 所有权标记；若同名表存在但无标记则拒绝覆盖。关闭此开关可停止刷新业务视图，不影响原五张汇总表。

三个业务视图在一次原子请求内写入并回读校验。专用汇总工作簿与明细工作簿之间没有跨文件事务，任一失败会返回失败，应重跑核对。自动刷新沿用现有汇总任务频率。

2026-09-23：业务视图扩展已部署并两次手动刷新回读成功；现有每日北京时间09:20调度核验为启用。原始记录未修改。内容资产与红人的有效链接仍需补充，不能把红人的整体成交收入分配到某条内容；广告素材连接需后续补充明确证据。14项本地单元测试通过。生产记录数量与个人信息不放入公开仓库。

配置 `SOURCE_SPREADSHEET_ID`、`SUMMARY_SPREADSHEET_ID`。使用已授权的公司 Cloud Run 运行身份，目标表需单独授予该机器账号编辑权限；默认模式源表只需读取权限，启用业务视图时源表也需编辑权限。服务必须保持 IAM 私有，建议单实例、并发1，由 Scheduler OIDC 调用 `POST /`。不要同时手动和定时执行，也不要在运行时部署新版本。
