...
- `google-ads-sheets-daily` 已启用，每天北京时间 09:10 同步 T-4。经用户确认，为现有运行机器账号仅在这个 Job 上授予 `roles/run.invoker`，未扩大 Google Ads 权限。
- 已通过 Scheduler 手动触发完整运行，两账户均返回 `SYNCED`。重跑后广告系列仍为 20 条、广告仍为 26 条、每日表现为 113 条（93 条历史 + 20 条新记录），记录 ID 与业务唯一键均无重复。
