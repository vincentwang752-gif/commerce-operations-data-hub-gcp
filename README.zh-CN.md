# 电商运营数据闭环：Shopify + GA4 + Google Sheets / Airtable + Google Cloud
...
现有明细工作簿增加三个自动关联业务视图：**客户跟进总览、订单来源总览、红人效果总览**。原表与同步字段保持不变；新视图把客户、订单、问卷、延保审核及红人触点放在一起阅读。缺少内容证据不猜测归因，跨币种不混加，问卷完成不等于延保已审核。开关、权限边界和口径见 [业务视图说明](services/dashboard-summary/README.md)。

新增 [看板汇总服务与运维说明](services/dashboard-summary/README.md)：从明细表生成经营、归因、问卷和网站日级汇总，写入独立汇总表，不含客户联系方式。2026-09-17 已部署私有服务并验证一次刷新，每天北京时间 09:20 的任务已启用；GA4 仍在 09:00 拉取 T-4。首次定时执行尚待验证，Looker Studio 图表仍在搭建，不能把汇总服务上线等同于完整看板交付。
...
- 问卷采集与延保资格分开处理：订单暂时匹配不到时，回答仍写入 Airtable 并进入人工匹配队列，不自动发放延保。
- 邮箱题必须提示客户填写下单邮箱并设为必填；`SYNCED` 只表示问卷已入库，订单资格需再看 `MATCHED` 或 `REVIEW_REQUIRED`。
...
问卷文案及订单匹配处理方式见 [VOC 问卷邮箱与订单匹配文案](docs/voc-survey-form-copy.md)。
...
- 新订单会按专属 Coupon、Click ID、UTM、Referrer 生成触点；证据都缺失时明确记录未知来源。每个订单只有一个最终触点计入收入，Collabs 平台确认归因优先。
- 当前只处理上线后的新订单，不自动回填历史归因触点。
